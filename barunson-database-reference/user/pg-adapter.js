...
class Statement {
  ...
  async run(...params) {
    ...
    if (/^\s*INSERT\s/i.test(sql) && !/RETURNING/i.test(sql)) {
      ...
      const table = m ? m[1] : '';
      const cols = m ? m[2] : '';
      const pkMap = { po_header:'po_id', po_items:'item_id', users:'user_id', vendors:'vendor_id', products:'id', receipts:'id', defects:'id', bom_header:'bom_id', bom_items:'bom_item_id', accessories:'id', work_orders:'wo_id', work_order_logs:'log_id', work_order_results:'id', product_notes:'product_code', product_field_history:'id', gift_sets:'id', gift_set_bom:'id', gift_set_transactions:'id', error_logs:'id', audit_log:'id', batch_master:'batch_id', batch_transactions:'txn_id', batch_inspections:'insp_id', incoming_inspections:'inspection_id', ncr:'ncr_id', ncr_logs:'id', notices:'id', cost_rates:'id', notice_reads:'id', vendor_notes:'id', approvals:'id', approval_lines:'id', sales_orders:'id', sales_order_items:'id', budgets:'id', daily_cash:'id', notifications:'id', journal_entries:'id', journal_entry_lines:'id', vendor_defect_reports:'id', receipt_items:'receipt_item_id', gift_assembly:'id', gift_assembly_materials:'id', china_shipment_log:'id', po_drafts:'id', trade_document:'id', inventory_snapshot:'product_code', sync_log:'id', order_history:'history_id' };
